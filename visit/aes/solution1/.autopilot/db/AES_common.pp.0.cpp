...
void KeyExpansion(BYTE RoundKey[176], BYTE Key[16]){
 ...
 VITIS_LOOP_10_1: for (i = 0; i < 16; i++){
  RoundKey[i] = Key[i];
 }


 VITIS_LOOP_15_2: for (i = 4; i < 4 * (10 + 1); i++){
  k = (i - 1) * 4;
...
void AddRoundKey(BYTE in[16],BYTE out[16] , BYTE RoundKey[16]){
 ...
 VITIS_LOOP_61_1: for(BYTE i = 0;i<16;i++){
