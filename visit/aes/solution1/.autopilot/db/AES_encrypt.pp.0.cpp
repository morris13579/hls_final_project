...
__attribute__((sdx_kernel("AES_ECB_encrypt", 0))) void AES_ECB_encrypt(hls::stream<BYTE>* plain ,hls::stream<BYTE>* encrypt , BYTE key[16] , unsigned long length);
...
void MixColumns(BYTE in[16],BYTE out[16]){
 BYTE i;
 BYTE Tmp, Tm, t;
 VITIS_LOOP_50_1: for(BYTE i = 0;i<16;i+=4){
  t = in[i];
...
#pragma HLS ARRAY_PARTITION variable=RoundKey factor=16
 ...
 BYTE state_39[16];
 BYTE state_40[16];
 VITIS_LOOP_117_1: for(BYTE i = 0 ;i<16;i++){
  state_0[i] = plain[i];
 }
#pragma HLS DATAFLOW
...
# 174 "../src/AES_encrypt.cpp"
 VITIS_LOOP_174_2: for(BYTE i = 0 ;i<16;i++){
  encrypt[i] = state_40[i];
 }
}


__attribute__((sdx_kernel("AES_ECB_encrypt", 0))) void AES_ECB_encrypt(hls::stream<BYTE>* plain ,hls::stream<BYTE>* encrypt , BYTE key[16] , unsigned long len) {_ssdm_SpecArrayDimSize(key, 16);
#pragma HLS TOP name=AES_ECB_encrypt
# 180 "../src/AES_encrypt.cpp"
...
#pragma HLS INTERFACE s_axilite port=return
 ...
 VITIS_LOOP_188_1: for(int i = 0 ;i<len;i+=16){
  BYTE in[16],out[16];
  VITIS_LOOP_190_2: for(int j =0;j<16;j++){
   in[j] = plain->read();
  }
  Cipher(in , out , RoundKey);
  VITIS_LOOP_194_3: for(int j =0;j<16;j++){
   encrypt->write(out[j]);
  }
 }
