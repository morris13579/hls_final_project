...
#ifndef _Cipher_state_2_V_H_
#define _Cipher_state_2_V_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "Cipher_state_2_V_memcore.h"

SC_MODULE(Cipher_state_2_V) {
...
protected:
    ...
    // buffer instances
    Cipher_state_2_V_memcore* buffer[BufferCount];
...
public:
    ~Cipher_state_2_V();
    SC_CTOR(Cipher_state_2_V) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "Cipher_state_2_V_memcore_U_";
            name += szTmp;

            buffer[i] = new Cipher_state_2_V_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "Cipher_state_2_V.trace");
            // ports
