...
__attribute__((sdx_kernel("AES_ECB_encrypt", 0))) void AES_ECB_encrypt(hls::stream<BYTE>* plain ,hls::stream<BYTE>* encrypt , BYTE* key , unsigned long length);
...
{
  VITIS_LOOP_9_1: for(BYTE i = 0;i<16;i++){
   out[i] = sbox[ in[i] ];
  }
}
...
void MixColumns(BYTE* in,BYTE* out){
 BYTE i;
 BYTE Tmp, Tm, t;
 VITIS_LOOP_48_1: for(BYTE i = 0;i<16;i+=4){
  t = in[i];
...
void Cipher(BYTE* plain, BYTE *encrypt,const BYTE RoundKey[176])
{
...
__attribute__((sdx_kernel("AES_ECB_encrypt", 0))) void AES_ECB_encrypt(hls::stream<BYTE>* plain ,hls::stream<BYTE>* encrypt , BYTE* key , unsigned long length) {
#pragma HLS TOP name=AES_ECB_encrypt
# 178 "AES_encrypt.cpp"
...
#pragma HLS INTERFACE s_axilite port=return
 ...
 VITIS_LOOP_186_1: for(int i = 0 ;i<length;i+=16){
  BYTE in[16],out[16];
  VITIS_LOOP_188_2: for(int j =0;j<16;j++){
   in[j] = plain->read();
  }
  Cipher(in , out , RoundKey);
  VITIS_LOOP_192_3: for(int j =0;j<16;j++){
   encrypt->write(out[j]);
  }
 }
}
