...
typedef BYTE ap_uint<8>;
# 28 "./aes.h"
void AES_ECB_encrypt(BYTE* in ,BYTE* out , BYTE* key);
void AES_ECB_decrypt(BYTE* in ,BYTE* out , BYTE* key);
# 3 "AES.cpp" 2
# 17 "AES.cpp"
...
static void KeyExpansion(BYTE* RoundKey, const BYTE* Key)
{
 ...
 VITIS_LOOP_55_2: for (i = 4; i < 4 * (10 + 1); ++i){
  k = (i - 1) * 4;
...
static void AddRoundKey(BYTE* in,BYTE* out , const BYTE* RoundKey)
{
 for(BYTE i = 0;i<16;i++){
...
static void SubBytes(BYTE* in,BYTE* out)
{
  for(BYTE i = 0;i<16;i++){
   out[i] = sbox(in[i]);
  }
...
static void ShiftRows(BYTE* in,BYTE* out)
{
...
static uint8_t xtime(BYTE x)
{
 return ((x<<1) ^ (((x>>7) & 1) * 0x1b));
}


static void MixColumns(BYTE* in,BYTE* out)
{
 BYTE i;
 BYTE Tmp, Tm, t;
# 166 "AES.cpp"
...
static void Cipher(BYTE* plain, BYTE *encrypt,const BYTE* RoundKey)
{
 BYTE state_0[16];
 BYTE state_1[16];
 BYTE state_2[16];
