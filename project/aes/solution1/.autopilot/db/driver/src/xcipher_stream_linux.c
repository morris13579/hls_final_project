...
typedef struct {
    ...
    XCipher_stream_uio_map maps[ MAX_UIO_MAPS ];
} XCipher_stream_uio_info;

/***************** Variable Definitions **************************************/
static XCipher_stream_uio_info uio_info;
...
static int uio_info_read_name(XCipher_stream_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XCipher_stream_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XCipher_stream_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XCipher_stream_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XCipher_stream_Initialize(XCipher_stream *InstancePtr, const char* InstanceName) {
	XCipher_stream_uio_info *InfoPtr = &uio_info;
